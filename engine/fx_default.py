...
class fx:
    ...
    def init_fx(self,parent,covers,index_cover_selected):
        for (index, cover) in enumerate(covers):
            ...
            if index == index_cover_selected:
                ...
                cover.set_position(parent.get_width()/2-cover.get_width()*BIG_SPACE+cover.get_width()*(index-index_cover_selected)*SPACE,parent.get_height()/2)
    ...
    def anim_dbl_click(self,actor):
        print "dbl_click"
        self.behaviours = []
        path = clutter.Path("M "+str(actor.get_x())+" "+str(actor.get_y())+" L "+str(actor.get_x())+" "+str(actor.get_y()-100)+" L "+str(actor.get_x())+" "+str(actor.get_y()))
        behavior = clutter.BehaviourPath(self.alpha,path)
