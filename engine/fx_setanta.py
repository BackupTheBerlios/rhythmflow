    ...
    def init_fx(self,parent,covers,index_cover_selected):
        for (index, cover) in enumerate(covers):
            ...
            if index == index_cover_selected:
                ...
                cover.set_position(parent.get_width()/2,parent.get_height()/2)
                cover.set_depth(0)    
            elif index == index_cover_selected - 1:
                cover.set_rotation(clutter.Y_AXIS,-ANGLE,0,0,0)
                cover.set_position(parent.get_width()/2-cover.get_width()*BIG_SPACE,parent.get_height()/2)
                cover.set_depth(-1)
            elif index == index_cover_selected + 1:
                cover.set_rotation(clutter.Y_AXIS,ANGLE,0,0,0)
                cover.set_position(parent.get_width()/2+cover.get_width()*BIG_SPACE,parent.get_height()/2)
                ...
                cover.set_scale(SCALE_OTHER,SCALE_OTHER)
                cover.set_position(parent.get_width()/2-cover.get_width()*BIG_SPACE+cover.get_width()*SPACE*(index-index_cover_selected),parent.get_height()/2)
            ...
            if (parent.get_index_cover_selected() > 0):
                ...
                for (index, cover) in enumerate(covers):
                    if index < index_cover_selected - 1:
                        cover.set_depth(index-index_cover_selected)
                        path = clutter.Path("M "+str(cover.get_x())+" "+str(cover.get_y())+" L "+str(cover.get_x()+cover.get_width()*SPACE)+" "+str(cover.get_y()))
                ...
                behavior = clutter.BehaviourRotate(clutter.Y_AXIS,ANGLE,0,self.alpha,clutter.ROTATE_CCW)
                behavior.apply(covers[index_cover_selected-1])
                self.behaviours.append(behavior)
                (cur_scale_x,cur_scale_y) = covers[index_cover_selected-1].get_scale()
                behavior = clutter.BehaviourScale(cur_scale_x,cur_scale_y,1.2,1.2,self.alpha)
                behavior.apply(covers[index_cover_selected-1])
                self.behaviours.append(behavior)
                path = clutter.Path("M "+str(covers[index_cover_selected-1].get_x())+" "+str(covers[index_cover_selected-1].get_y())+" L "+str(parent.get_width()/2)+" "+str(parent.get_height()/2))
                behavior = clutter.BehaviourPath(self.alpha,path)
                behavior.apply(covers[index_cover_selected-1])
                self.behaviours.append(behavior)
             
                covers[index_cover_selected].set_depth(-1)
                behavior = clutter.BehaviourRotate(clutter.Y_AXIS,0,360-ANGLE,self.alpha,clutter.ROTATE_CCW)
                ...
                behavior.apply(covers[index_cover_selected])
                self.behaviours.append(behavior)
                path = clutter.Path("M "+str(covers[index_cover_selected].get_x())+" "+str(covers[index_cover_selected].get_y())+" L "+str(parent.get_width()/2+covers[index_cover_selected].get_width()*BIG_SPACE+covers[index_cover_selected].get_width()*(1)*SPACE)+" "+str(covers[index_cover_selected].get_y()))
                behavior = clutter.BehaviourPath(self.alpha,path)
                behavior.apply(covers[index_cover_selected])
                self.behaviours.append(behavior)
    ...
    def anim_right(self,parent,covers,index_cover_selected):
        ...
        if self.anim_finished :
            if (parent.get_index_cover_selected() < len(covers)-1):
                ...
                covers[index_cover_selected].set_depth(-1) 
                behavior = clutter.BehaviourRotate(clutter.Y_AXIS,0,ANGLE,self.alpha,clutter.ROTATE_CW)
                behavior.apply(covers[index_cover_selected])
                self.behaviours.append(behavior)
                (cur_scale_x,cur_scale_y) = covers[index_cover_selected].get_scale()
                behavior = clutter.BehaviourScale(cur_scale_x,cur_scale_y,1,1,self.alpha)
                ...
                for (index, cover) in enumerate(covers):
                    if index > index_cover_selected + 1:
                        cover.set_depth(index_cover_selected-index-1)
                        path = clutter.Path("M "+str(cover.get_x())+" "+str(cover.get_y())+" L "+str(cover.get_x()-cover.get_width()*SPACE)+" "+str(cover.get_y()))
                        behavior = clutter.BehaviourPath(self.alpha,path)
                        behavior.apply(cover)
                        self.behaviours.append(behavior)
